...
@section ('data')
<?php ### customized data
    $pageTitle      = 'Edit peminjaman alat';
...
<div class="card mt-2">
    ...
    <form action="{{ route($formRouteUpdate, $dataTool->id) }}" method="post" enctype="multipart/form-data" data-parsley-validate novalidate>
        @csrf
        @method ('PUT')
        <!-- hidden data -->
                ...
                <div class="row m-0">
                    <div class="col-6">
                        <div class="form-group">
                            <label>Nama alat <small class="c-red">*</small></label>
                            <input type="text" name="name" class="form-control{{ $errors->has('name') ? ' has-error' : '' }}" value="{{ old('name') ?? $dataTool->name }}" placeholder="Nama alat" required>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="form-group">
                            <label>Kode alat <small class="c-red">*</small></label>
                            <input type="text" name="code" class="form-control{{ $errors->has('code') ? ' has-error' : '' }}" value="{{ old('code') ?? $dataTool->code }}" placeholder="Kode alat" required>
                        </div>
                    </div>
                </div>
            </div>
