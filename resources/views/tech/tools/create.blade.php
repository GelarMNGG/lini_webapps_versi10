...
@section ('data')
<?php ### customized data
    $pageTitle      = 'Ajukan peminjaman alat';
                ...
                <div class="row m-0">
                    <div class="col-md">
                        <div class="form-group">
                            ...
                            <input type="text" name="name" class="form-control{{ $errors->has('name') ? ' has-error' : '' }}" value="{{ old('name') ?? '' }}" placeholder="Nama alat" required>
                        </div>
                    </div>
                    <div class="col-md col-6">
                        <label for="">Kode alat</label>
                        <select name="code" class="form-control select2{{ $errors->has('code') ? ' has-error' : '' }}">
                            <?php 
                                if(old('code') != null) {
                                    $code = old('code');
                                }else{
                            ...
                            @if($code != null)
                                @foreach($toolCodes as $toolCode)
                                    @if($toolCode->id == $code)
                                    <option value='{{ $toolCode->id }}'>{{ strtoupper($toolCode->code) }}</option>
                                    @endif
                                @endforeach
                            @else
