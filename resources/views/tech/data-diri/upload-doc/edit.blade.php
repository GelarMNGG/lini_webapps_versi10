...
<?php ### customized data
    ...
    $formRouteUpdate = 'tech-input-doc.update';
...
<div class="card mt-2">
    ...
    <form class="w-100" action="{{ route($formRouteUpdate, $docData->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
            ...
            <div class="row m-0">
                <div class="col-md form-group{{ $errors->has('doc_type') ? ' has-error' : '' }} ">
                    ...
                    <select name="doc_type" class="form-control" placeholder="Tipe dokumen" required>
                        <?php 
                            ...
                            }elseif(isset($docData->doc_type)){
                                $dataTypes = $docData->doc_type;
                            }else{
                                $dataTypes = NULL;
                            }
                        ?>
                ...
                <div class="col-md{{ $errors->has('image') ? ' has-error' : '' }}">
                    <div class="card-box">
                        ...
                        <input type="file" name="image" class="dropify" data-max-file-size="1M" data-default-file="{{ asset('img/upload-doc/tech/'.$docData->image) }}" />
                    </div>
                </div>
            </div>
        </div>
