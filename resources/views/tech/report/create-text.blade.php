...
<?php ### customized data
    ...
    //image comments
    $formTextCommentsStore = 'project-text-comment-tech.store';
?>
@endsection
    ...
    <div class="content mt-2">
        ...
        <div class="container-fluid">
            <div class="flash-message">
                ...
                @if(!isset($dataProjectText))
                    <p class="alert alert-danger">Anda belum melengkapi deskripsi yang diperlukan.<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a></p>
                @endif
                ...
                <div class="col-md-12">
                        ...
                        <div class="panel-body">
                                ...
                                <div class="col-md-12">
                                    @foreach($dataSubcategory as $dataSubcat)
                                        @if($dataSubcat->id == $subcatId)
                                            ...
                                            <div class="row m-0">
                                                @if(isset($subcatsTextByCatCount))
                                                    @if(isset($dataProjectText))
                                                        <form class="col-md" action="{{ route($formRouteUpdate, $dataProjectText->id) }}" method="POST" enctype="multipart/form-data">
                                                            @csrf
                                                            @method('PUT')

                                                            <div class="row">
                                                                <div class="col-md-12{{ $errors->has('text') ? ' has-error' : '' }}">
                                                                    <textarea type="text" class="form-control" name="text" cols="10" rows="17" minlength="10">{{ old('text') !== null ? old('text') : $dataProjectText->text }}</textarea>
                                                                </div>
                                                                <div class="w-100"></div>
                                                                ...
                                                                @if(!isset($dataProjectText->submitted_at) && $dataProjectTextStatus->countApproved < 1 && $dataProjectTextStatus->countPMApproved < 1)
                                                                    <div class="col-md d-print-none">
                                                                        <button type="submit" class="btn btn-danger mt-1" name="submit"> Simpan perubahan</button>
                                                                    </div>
                                                                @endif
                                                            </div>
                                                        </form>
                                                    @else
                                                        <form class="col-md" action="{{ route($formRouteStore, 'project_id='.$projectTask->project_id.'&task_id='.$projectTask->id) }}" method="POST" enctype="multipart/form-data">
                                                            @csrf
                                                            <div class="row">
                                                                <div class="col-md form-group{{ $errors->has('text') ? ' has-error' : '' }}">
                                                                    <textarea type="text" class="form-control" name="text" cols="10" rows="15" minlength="10" required>{{ old('text') !== null ? old('text') : '' }}</textarea>
                                                                </div>
                                                                <div class="w-100"></div>
                            ...
                            <div class="d-print-none">
                                <div class="float-right">
                                    ...
                                    @if(isset($dataProjectText) && $dataProjectTextStatus->status < 2  && $dataProjectTextStatus->countApproved < 1 && $dataProjectTextStatus->countPMApproved < 1)
                    ...
                    */ ?>
                    <div class="alert alert-{{ $css_2 }}"> 
                        <strong>[ <i class="fas fa-{{ $fa_code_2 }}"></i> ]</strong> Laporan text disetujui oleh <span class="{{ $css_text_2 }}">Admin Document</span>.
                    ...
                    <div class="alert alert-{{ $css_1 }}">
                        <strong>[ <i class="fas fa-{{ $fa_code_1 }}"></i> ]</strong> Laporan text diajukan oleh <span class="{{ $css_text_1 }}">Teknisi</span>.
                    ...
                    <div class="d-print-none">
                        <form method="post" action="{{ route($formTextCommentsStore) }}" enctype="multipart/form-data" class="card-box">
                            @csrf
                            ...
                            <input value="{{ $projectTemplate->template_id }}" name="template_id" hidden>
                            <input value="{{ $subcatId }}" name="{{ $subcatName }}" hidden>
                            <input value="{{ $dataProjectTextStatus->id }}" name="prt_id" hidden>
                            <!-- comment data -->
                            <div class="pt-1 float-right">
                                <button type="submit" name="submit" class="btn btn-primary btn-sm waves-effect waves-light" @if($dataProjectTextStatus->countPMApproved > 0) ?? disabled @endif>Kirim komentar</button>
                            </div>
                        ...
                        @if($dataProjectTextStatus->commentsCount > 0)
