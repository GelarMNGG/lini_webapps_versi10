...
<?php ### customized data
    ...
    $formRequestStore = 'admin-cities.store';
?>
@endsection
...
<div class="card mt-2">
    ...
    <form class="w-100" action="{{ route($formRequestStore) }}" method="POST" enctype="multipart/form-data">
        <div class="card-body bg-gray-lini-2">
            @csrf
            <div class="row m-0">
                <div class="col-md form-group kota-asal-box">
                    <label for="">Kota asal <small class="c-red">*</small></label>
                    <input name="name" class="form-control{{ $errors->has('name') ? ' has-error' : '' }}" value="{{ ucwords(strtolower(old('name'))) }}">
                </div>
                <div class="col-md form-group{{ $errors->has('code') ? ' has-error' : '' }}">
                    ...
                    <select id="code" name="code" class="form-control select2" required>
                        <?php 
                            if (old('code')) {
                                $dataPropinsi = old('code');
                            }elseif (isset($propinsiId)) {
                                $dataPropinsi = $propinsiId;
                            }else{
                                $dataPropinsi = NULL;
                            }
                        ?>
                        @if($dataPropinsi != NULL)
                            @foreach ($provincesDatas as $data2)
                                @if($data2->id == $dataPropinsi)
