...
<?php ### customized data
    ...
    $formRouteIndex = 'admin-pr.index';
    $formRouteUpdate= 'admin-pr.update';
?>
@endsection
                ...
                <div class="col-md-12">
                    <div class="card-box">
                        ...
                        <div class="panel-heading text-center text-uppercase">
                            <h3>{{ $pageTitle }}</h3>
                            Proyek: <strong><span class="text-info">{{ strtoupper($dataPR->project_name) }}</span></strong>
                            <br>Task: <strong><span class="text-danger">{{ isset($dataPR->task_name) ? strtoupper($dataPR->task_name) : 'Belum ada task' }}</span></strong>
                        </div>
                        <hr>
                                ...
                                <div class="float-right">
                                    ...
                                    @if($dataPR->status == 2)
                                        <form action="{{ route($formRouteUpdate, $dataPR->id) }}" style="display:inline-block" method="post" enctype="multipart/form-data" data-parsley-validate novalidate>
                                            ...
                                            <button type="submit" class="btn btn-icon waves-effect waves-light btn-danger" name="submit"><i class='fas fa-check' title='done'> </i> Setujui</button>
                                        </form>
                                        <form action="{{ route($formRouteUpdate, $dataPR->id) }}" style="display:inline-block" method="post" enctype="multipart/form-data" data-parsley-validate novalidate>
                                            @csrf
                                            @method('PUT')
                                            <input class="form-control" type="number" name="status" value="1" hidden>
                                            <button type="submit" class="btn btn-icon waves-effect waves-light btn-info" name="submit"><i class='fas fa-edit' title='done'> </i> Minta revisi</button>
                                        </form>
                                        <!-- <a href="#" class="btn btn-icon waves-effect waves-light btn-warning" name="tolak"><i class='fas fa-times' title='done'> </i> Tolak</a> -->
                                    @endif
