...
<?php ### customized data
    ...
    $formRouteEdit    = 'task-leaders.edit';
    $formRouteUpdate    = 'task-leaders.update';

    //task comment
    $formCommentsCreate = 'task-leaders-comment.store';

    //upload route
    $formFileStore    = 'task-leaders-file.store';
    $formFileDestroy    = 'task-leaders-file.destroy';
    
    //upload comment file route
    $formCommentFileStore    = 'task-leaders-comment-file.store';
    $formCommentFileDestroy    = 'task-leaders-comment-file.destroy';

    //pic route
    $formPicCreate    = 'task-leaders-pic.create';
    $formPicEdit    = 'task-leaders-pic.edit';
    $formPicDestroy    = 'task-leaders-pic.destroy';

    //todo
    $formTodoCreate = 'task-leaders-todo.create';
    $formTodoEdit = 'task-leaders-todo.edit';
    $formTodoShow = 'task-leaders-todo.show';
    $formTodoDestroy = 'task-leaders-todo.destroy';
    
    //todo file
    $formTodoFileDestroy = 'task-leaders-todo-file.destroy';
...
@if($taskData->publisher_id == $userId && $taskData->publisher_type == $userType || in_array($userDepartment,$taskDataReceiverDepartments) || Auth::user()->department_id == 11)
    <div class="card mt-2">
        <div class="card-header text-center bb-orange">
            ...
            ?>
            <span class="badge badge-{{ $multiLevelStatusCSS }}" style="margin-left:10px;">{{ ucfirst($taskData->task_level_title) }}</span>
            <br><span class="small">
                Inisiated by 
                    ...
                    <div class="row">
                        <div class="col-md">
                            <div class="card-box task-detail">
                                                    ...
                                                    @foreach($leadersTodos as $todo)
                                                        @if($todo->department_id == $userReceiver->department_id)
                                                            <div class="col-md-12 mb-1">
                                                                ...
                                                                @if(Auth::user()->department_id == $userReceiver->department_id || (Auth::user()->id == $todo->requester_id && Auth::user()->user_type == $todo->requester_type) || Auth::user()->department_id == $taskData->publisher_department)
                                                        ...
                                                        @if($picData->task_id == $taskData->id && $picData->department_id == $userReceiver->department_id)
                                                            <div class="w-100 mb-1">
                                                                ...
                                                                <form action="{{ route($formPicDestroy, $picData->id) }}" style="display:inline;" method="POST">
                                                                    ...
                                                                    @if(Auth::user()->department_id == $userReceiver->department_id || Auth::user()->department_id == $taskData->publisher_department)
                                                                        <button type="submit" class="badge badge-danger" onclick="return confirm('Apakah Anda yakin akan menghapus data ini?')">Hapus</button>
                                                                    @endif
                                                                </form>
                                                            </div>
                                ...
                                @endforeach
                                <!-- supporting document -->
                                @if(Auth::user()->department_id == $taskData->publisher_department)
                                    <div class="row m-0">
                                ...
                                <!-- supporting document -->
                                <!-- other documents -->
                                @if(count($commentFiles) > 0 || count($leadersTodoFiles) > 0)
                                    <div class="row m-0">
                                                    ...
                                                    @foreach($commentFiles as $commentLeaderFiles)
                                                        @if($commentLeaderFiles->task_id == $taskData->id)
                                                            <div class="col-md-12">
                                                                ...
                                                                <a href="{{ asset('img/upload-doc/task-leaders/'.$commentLeaderFiles->image) }}" class="display:inline-block;">[{{ ucfirst($commentLeaderFiles->image) }}]</a>
                                                    ...
                                                    <div class="row">
                                                        <div class="col-md{{ $errors->has('image') ? ' has-error' : '' }}">
                                                            <div class="card-box">
                                                                ...
                                                                <input type="file" name="image" class="dropify" data-max-file-size="4M" data-default-file="{{ asset('img/comment-file/task-leaders/default.png') }}" required />
                                                            </div>
                                                        </div>
                                                        <div class="w-100"></div>
                                                        <div class="col-md mt-2 mb-2">
