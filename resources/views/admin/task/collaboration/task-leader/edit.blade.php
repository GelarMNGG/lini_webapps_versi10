...
@section ('data')
<?php ### customized data
    $pageTitle      = 'Edit multi-colaborative task'; 
        ...
        <div class="card-body bg-gray-lini-2">
            ...
            <div class="row m-0">
                ...
                <div class="col-md form-group">
                    ...
                    <?php
                        if(old('receiver_department[]') != null) {
                            $taskDepartments = old('receiver_department[]');
                        }if(isset($taskDataDepartment) != null) {
                            $taskDepartments = $taskDataDepartment;
                        }else{
                            $taskDepartments = null;
                        }
                    ?>
                    <select class="select2 select2-multiple" name="receiver_department[]" multiple="multiple" multiple data-placeholder="Choose ...">
                        @if(isset($departmentDatas))
                            @foreach($departmentDatas as $departmentData)
                                @if(!in_array($departmentData->id,$taskDataDepartment))
                                    <option value="{{ $departmentData->id }}">{{ ucwords($departmentData->name) }}</option>
                                @endif
                            @endforeach
                        @endif
                    </select>
                    @if ($taskDepartments != null)
                        <span class="small"> Departemen yang sudah berkolaborasi: </span>
                        @foreach($departmentDatas as $departmentData)
                            @if (in_array($departmentData->id, $taskDepartments))
                                <span class="badge badge-danger">{{ ucwords($departmentData->name) }}</span>
                            @endif
                        @endforeach
                    @endif
                </div>
