...
<div class="card mt-2">
    <div class="card-header text-center text-uppercase bb-orange">
        @if ($userProfile->firstname !== null)
            {{ ucfirst($pageTitle) }} | <strong>{{ ucfirst($userProfile->firstname).' '. ucfirst($userProfile->lastname) }}</strong>
        @else
            {{ ucfirst($pageTitle) }} | <strong>{{ ucfirst($userProfile->name) }}</strong>
        @endif
    </div>
                                    ...
                                    <select id="department_id" name="department_id" class="form-control select2" required>
                                        @if (!empty(old('department_id')) || !empty($userProfile->department_id))
                                            @foreach($departments as $department)
                                                ...
                                                @elseif($department->id == $userProfile->department_id)
                                                    <option value="{{ $department->id }}">{{ ucwords($department->name)}}</option>
                                                @endif
                                            @endforeach
                                            @foreach($departments as $department)
