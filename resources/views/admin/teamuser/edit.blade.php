...
<?php ### customized data
    ...
    $formRouteUpdate= 'teamuser.update';
?>
@endsection
    ...
    <form action="{{ route($formRouteUpdate, $userProfile->id) }}" method="post" enctype="multipart/form-data" data-parsley-validate novalidate>
        ...
        <div class="card-body bg-gray-lini-2">
            <div class="row">
                        ...
                        <select id="user_level" name="user_level" class="form-control select2">
                            ...
                            @if ($user_level != null)
                                @foreach($userLevels as $userLevel)
                                    @if($userLevel->id == $user_level || $userLevel->role == $user_level)
                            ...
                            <select name="department_id" class="form-control select2">
                                <?php 
                                    ...
                                    if(old('department_id') != null) {
                                        $user_department = old('department_id');
                                    }elseif(isset($userProfile->department_id)){
                                ...
                                @if ($user_department != null)
                                    @foreach($departments as $department)
                                        @if($department->id == $user_department)
                ...
                @if(Auth::user()->department_id == 5 && isset($userProfile->deleted_at))
                    <div class="col-md-12">
                        <div class="alert alert-warning">
                            <label for="">User ini sudah terhapus dari database pada <span class="text-danger">{{ date('l, d F Y', strtotime($userProfile->deleted_at)) }}</span>. Apakah Anda akan merecover akun ini?</label>
                            <select id="deleted_at" name="deleted_at" class="form-control" required>
                                <option value="0">No</option>
                                <option value="1">Yes. Recover!</option>
                            </select>
                        </div>
                    </div>
                @endif
