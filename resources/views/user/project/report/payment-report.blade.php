...
<?php ### customized data
    ...
    //route CA
    $formRouteCA= 'user-projects.show';

    //payment summary
    $formRoutePaymentSummaryIndex = 'user-project-payment-summary.index';
    $formRoutePaymentSummaryCreate = 'user-project-payment-summary.create';
    $formRoutePaymentSummaryUpdate = 'user-project-payment-summary.update';
    $formRoutePaymentSummaryDestroy = 'user-project-payment-summary.destroy';
?>
@endsection
                    ...
                    <div class="card-box">
                        ...
                        <div class="panel-body">
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        ...
                                                        @elseif($total < 0)
                                                            <span class="{{ $cssTotal }}"><strong>(Rp. {{ number_format($total ?? 0) }})</strong></span>
                                                        @else
                                                            <span class="text-info"><strong>0</strong></span>
                                                        @endif
                                                    </td>
                                                </tr>
                                            </tbody>
                            ...
                            <div class="d-print-none">
                                <div class="float-right">
                                    <a href="javascript:window.print()" class="btn btn-dark waves-effect waves-light mb-1 ml-1"><i class="fa fa-print"></i></a>

                                    @if($total < 0)
                                        <a href="{{ route($formRoutePaymentSummaryCreate,'project_id='.$projectTaskInfo->project_id.'&task_id='.$projectTaskInfo->id) }}" type="button" class="btn btn-danger mb-1 ml-1">Upload bukti transfer</a>
                                    @endif

                                    @if(Auth::user()->user_level == 3)
                                        <a href="{{ route($formRouteBack,$projectTaskInfo->project_id) }}" type="button" class="btn btn-secondary mb-1 ml-1">Kembali</a>
                                    @else
                                        <a href="{{ route($formRouteBack,$projectTaskInfo->id) }}" type="button" class="btn btn-secondary mb-1 ml-1">Kembali</a>
                                    @endif
                                ...
                                <div class="row">
                                    <div class="col-md mt-3">
                                        ...
                                        @else
                                            @if($total < 0)
                                                <div class="alert alert-success">Nilai total dana yang harus dikembalikan Teknisi kepada LINI adalah sebesar <span class="{{ $cssTotal }}"><strong>Rp. {{ number_format(abs($total) ?? 0) }}</strong></span>.</div>
                                            @elseif($total > 0)
                                                <div class="alert alert-warning">Nilai total yang akan ditransfer LINI ke teknisi adalah sebesar <span class="{{ $cssTotal }}"><strong>Rp. {{ number_format(abs($total) ?? 0) }}</strong></span></div>
                                            @else
                                            ...
                                            <div class="col-md">
                                                ...
                                                <div class="col-md mt-1 text-center">
                                                    <div class="approve-button">
                                                        ...
                                                        <form action="{{ route($formRoutePaymentSummaryUpdate,$paymentSummaryData->id) }}" style="display:inline-block;"  method="post" enctype="multipart/form-data" data-parsley-validate novalidate>
                                                            ...
                                                            @if($paymentSummaryData->status == 1 && Auth::user()->user_level == 6 && $paymentSummaryData->reject_status == 0)
                                                            ...
                                                            @endif
                                                            @if($paymentSummaryData->status == 2 && Auth::user()->user_level == 3 && $paymentSummaryData->reject_status == 0)
                                                            ...
                                                            <!-- reject -->
                                                            @if($paymentSummaryData->reject_status == 0)
                                                                <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#reject{{ $paymentSummaryData->id }}"><i class="fas fa-times"></i> reject</button>
                                                            @endif

                                                            <!-- Modal -->
                                                            <div class="modal fade" id="reject{{ $paymentSummaryData->id }}" tabindex="-1" role="dialog" aria-labelledby="rejectModel" aria-hidden="true">
                                                                <div class="modal-dialog-reject modal-dialog-centered" role="document">
                                                                        ...
                                                                        <div class="modal-body text-center">
                                                                            ...
                                                                            <div id="rejectModel">
                                                                                <form action="{{ route($formRoutePaymentSummaryUpdate,$paymentSummaryData->id) }}" style="display:inline-block;"  method="post" enctype="multipart/form-data" data-parsley-validate novalidate>
                                                                                    ...
                                                                                    <div class="row{{ $errors->has('reject_note') ? ' has-error' : '' }}">
                                                                                        <!-- hidden data -->
                                                                                        <input type="text" name="project_id" value="{{ $projectTaskInfo->project_id }}" hidden>
                                                                                        <input type="text" name="task_id" value="{{ $projectTaskInfo->id }}" hidden>
    
                                                                                        <input type="text" name="reject_status" value="1" hidden>

                                                                                        <input type="text" name="reject_note" class="form-control" value="{{ old('reject_note')}}" placeholder="Alasan penolakan">

                                                                                        <button type="submit" class="btn btn-icon waves-effect waves-light btn-success mt-1 mr-1" name="submit">Reject </button>
    
                                                                                        <a href="{{ route($formRoutePaymentSummaryIndex, 'project_id='.$projectTaskInfo->project_id.'&task_id='.$projectTaskInfo->id) }}" class="btn btn-danger mt-1"><i class='fas fa-times' title='Reject'> </i> Close</a>
                                                                                    </div>
                                                                                </form>
                                                                            </div>
                                                                        </div>
