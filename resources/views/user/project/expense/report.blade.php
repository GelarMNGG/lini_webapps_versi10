...
<?php ### customized data
    ...
    //expense report
    $formRouteReportUpdate= 'user-projects-expense-report.update';
?>
@endsection
                    ...
                    <div class="panel-body">
                        <div class="clearfix">
                            <div class="float-left">
                                <span>Nama pemohon: {{ ucwords($userProfile->firstname).' '.ucwords($userProfile->lastname) }}</span>
                                <br>Task: <span class="text-uppercase">{{ $projectTaskInfo->name }}</span>
                            </div>
                            <div class="float-right">
                                <h4>No task #<strong><span class="text-uppercase">{{ $projectTaskInfo->number }}</span></strong> </h4>
                        ...
                        <div class="d-print-none">
                            <div class="float-right">
                                ...
                                <a href="{{ route($formRouteIndex,'project_id='.$projectTaskInfo->project_id.'&task_id='.$projectTaskInfo->id) }}" type="button" class="btn btn-secondary">Kembali</a>

                                @if(Auth::user()->user_level == 3)
                                    @if($projectExpensesReport->submitted_at != null && $projectExpensesReport->canceled_at == null && $projectExpensesReport->approved_at != null && $projectExpensesReport->approved_by_pm_at == null)
                                        <form action="{{ route($formRouteReportUpdate, $projectExpensesReport->id.'?project_id='.$projectTaskInfo->project_id.'&task_id='.$projectTaskInfo->id) }}" style="display:inline-block" method="post" enctype="multipart/form-data" data-parsley-validate novalidate>
                                            @csrf
                                            @method('PUT')
                                            ...
                                            <button type="submit" class="btn btn-icon waves-effect waves-light btn-success" name="submit"><i class='fas fa-paper-plane' title='done'> </i> Setujui laporan</button>
                                        </form>
                                    @endif
                                @endif

                                @if(Auth::user()->user_level == 6)
                                    @if($projectExpensesReport->submitted_at != null && $projectExpensesReport->canceled_at == null && $projectExpensesReport->approved_at == null)
                                        <form action="{{ route($formRouteReportUpdate, $projectExpensesReport->id.'?project_id='.$projectTaskInfo->project_id.'&task_id='.$projectTaskInfo->id) }}" style="display:inline-block" method="post" enctype="multipart/form-data" data-parsley-validate novalidate>
                                            @csrf
                                            @method('PUT')
                                ...
                                @if($projectExpensesReport->canceled_at != null)
                                    <form action="{{ route($formRouteReportUpdate, $projectExpensesReport->id.'?project_id='.$projectTaskInfo->project_id.'&task_id='.$projectTaskInfo->id) }}" style="display:inline-block" method="post" enctype="multipart/form-data" data-parsley-validate novalidate>
                                        ...
                                        <button type="submit" class="btn btn-icon waves-effect waves-light btn-success" name="submit"><i class='fas fa-paper-plane' title='done'> </i> Setujui pembatalan laporan</button>
                                    </form>
                                @endif
                            ...
                            <div class="row">
                                <div class="col-md mt-3">
                                    @if($projectExpensesReport->status == 4)
                                        <div class="alert alert-success">PM telah menyetujui laporan pengeluaran.</div>
                                    @elseif($projectExpensesReport->status == 3)
                                        <div class="alert alert-success">QC Expenses telah menyetujui laporan pengeluaran.</div>
                                    @elseif($projectExpensesReport->status == 2)
                                        ...
                                        @else
                                            <div class="alert alert-warning">Laporan menunggu review.</div>
                                        @endif
                                    @else
