...
<?php ### customized data
    ...
    ###form action route
    $formRouteIndex     = 'user-task-internal.index';
    $formRouteEdit    = 'user-task-internal.edit';
    $formRouteUpdate    = 'user-task-internal.update';

    //user-task comment
    $formCommentsCreate = 'user-task-internal-comment.store';

    //upload route
    $formFileStore    = 'user-task-internal-file.store';
    $formFileDestroy    = 'user-task-internal-file.destroy';
    
    //upload comment file route
    $formCommentFileStore    = 'user-task-internal-comment-file.store';
    $formCommentFileDestroy    = 'user-task-internal-comment-file.destroy';

    //pic route
    $formPicCreate    = 'user-task-internal-pic.create';
    $formPicEdit    = 'user-task-internal-pic.edit';
    $formPicDestroy    = 'user-task-internal-pic.destroy';

    //todo
    $formTodoCreate = 'user-task-internal-todo.create';
    $formTodoEdit = 'user-task-internal-todo.edit';
    $formTodoShow = 'user-task-internal-todo.show';
    $formTodoDestroy = 'user-task-internal-todo.destroy';
    
    //todo file
    $formTodoFileDestroy = 'user-task-internal-todo-file.destroy';
                                        ...
                                        @if($picData->task_id == $taskData->id)
                                            <div class="row m-0">
                                                <h4 class="text-secondary"><span class="text-danger">{{ ucfirst($picData->pic_firstname).' '.ucfirst($picData->pic_lastname) }}</span> 
                                                    @if(in_array(Auth::user()->id,$coAdminDatas) || (Auth::user()->id == $taskData->publisher_id && Auth::user()->user_type == $taskData->publisher_type))
                                                        | 
