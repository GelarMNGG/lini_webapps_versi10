    ...
    <form class="w-100" action="{{ route($formRouteStore) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="col-md-6">
                    ...
                    @if(Auth::user()->company_id == 1 && Auth::user()->department_id == 1)
                        <div class="row m-0 form-group{{ $errors->has('description') ? ' has-error' : '' }}">
                            ...
                            <textarea name="description" class="form-control" cols="10" rows="11">{{ old('description') }}</textarea>
                        </div>
                    @else
                        <div class="row">
                                ...
                                <select id="department_id" name="department_id" class="form-control select2{{ $errors->has('department_id') ? ' has-error' : '' }}" required>
                                    @if (!empty(old('department_id')))
                                        @foreach ($departmentDatas as $departmentData)
                                            @if ($departmentData->id == old('department_id'))
                ...
                <div class="col-md-6">
                    <div class="row">
                        ...
                        <div class="col-md form-group">
                            ...
                            <div class="input-group">
                                <input id="timepicker" name="event_end" type="text" class="form-control" value="{{ old('event_end') ?? '' }}" required>
                                <div class="input-group-append">
                                    <span class="input-group-text"><i class="mdi mdi-clock-outline"></i></span>
                                </div>
                            </div><!-- input-group -->
                    ...
                    <div class="form-group{{ $errors->has('image') ? ' has-error' : '' }}">
                        <input type="file" name="image" class="dropify" data-max-file-size="1M" data-default-file="{{ asset('img/minutes/user/default.png') }}"  />
                    </div>
                </div>
            </div>
            <div class="row m-0">
        ...
        <div class="card-body">
            <div class="col-md">
                ...
                <a href="{{ route($formRouteIndex,'skin='.$skin) }}" type="button" class="btn btn-blue-lini">Kembali</a>
            </div>
        </div>
    </form>
